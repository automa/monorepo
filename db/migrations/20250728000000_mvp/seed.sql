...
VALUES
  ...
  (2, 'github', '1299', 'm@mitchellh.com', 'refresh_token');
...
VALUES
  ...
  (5, 'github-runners', 'Changes GitHub CI configuration to use Depot runners', 'https://depot.dev/assets/brand/1693758816/depot-icon-on-light.svg', 'scheduled', 'http://localhost:5004/hooks/automa', 'atma_whsec_github-runners', 'https://depot.dev', NOW(), TRUE, TRUE, FALSE, ARRAY['.github/workflows'], ARRAY['.github/workflows']),
  (1, 'posthog', 'Adds code to track a new analytics event using Posthog', 'https://posthog.com/brand/posthog-logomark.svg', 'manual', 'http://localhost:5006/hooks/automa', 'atma_whsec_posthog', 'https://posthog.com', NOW(), FALSE, TRUE, TRUE, DEFAULT, ARRAY['data']),
  (6, 'gemini', 'Google''s coding agent', 'https://automa.app/logos/gemini.svg', 'manual', 'http://localhost:5011/hooks/automa', 'atma_whsec_gemini', 'https://deepmind.google/models/gemini/', NOW(), FALSE, FALSE, TRUE, DEFAULT, DEFAULT),
  (1, 'amp', 'Sourcegraph''s coding agent', 'https://automa.app/logos/amp.svg', 'manual', 'http://localhost:5012/hooks/automa', 'atma_whsec_amp', 'https://ampcode.com', NOW(), FALSE, TRUE, FALSE, DEFAULT, DEFAULT);
...
VALUES
  ...
  (1, 'Set default repo & bot during task creation', FALSE, 'f', NOW(), 'submitted'),
  (1, 'Set default repo & bot during task creation', FALSE, 'g', NOW(), 'submitted'),
  (1, 'Track "User Login Attempted" event', FALSE, 'h', NOW(), DEFAULT),
  (1, 'Track "User Logout Attempted" event', FALSE, 'i', NOW(), DEFAULT),
  (1, 'Provision the node.js template repo', FALSE, 'j', NOW(), 'skipped');
...
VALUES
  ...
  (3, NOW(), NULL, NULL, 7, NULL, 'bot', '{}'),
  (3, NOW(), NULL, NULL, 7, 3, 'activity', '{}'),
  (4, NOW(), NULL, 1, NULL, NULL, 'repo', '{}'),
  (4, NOW(), NULL, NULL, 7, NULL, 'bot', '{}'),
  (5, NOW(), NULL, 1, NULL, NULL, 'repo', '{}'),
  (5, NOW(), NULL, NULL, 7, NULL, 'bot', '{}'),
  (5, NOW(), NULL, 1, 7, NULL, 'proposal', '{ "prId": 107, "prNumber": 7, "prTitle": "Use depot managed runners for github actions", "prState": "closed", "prMerged": false, "prHead": "automa:depot/github-runners/5", "prBase": "master" }'),
  (5, NOW(), 1, NULL, NULL, 4, 'activity', '{}'),
  (6, NOW(), 2, NULL, NULL, NULL, 'message', '{ "content": "We should only do this if the options are not already selected because of the URL.\n\nIn the file named `TaskCreate.tsx`, we can use `useEffect` along with `setValue`. Don''t use `getValues`." }'),
  (6, NOW(), 2, NULL, NULL, NULL, 'origin', '{ "orgId": 1 }'),
  (6, NOW(), 2, 1, NULL, NULL, 'repo', '{}'),
  (6, NOW(), 2, NULL, 6, NULL, 'bot', '{}'),
  (6, NOW(), NULL, 1, 6, NULL, 'proposal', '{ "prId": 103, "prNumber": 3, "prTitle": "Auto-select bot & repo if none specified", "prState": "open", "prMerged": false, "prHead": "automa:claude-code/6", "prBase": "master" }'),
  (7, NOW(), 2, NULL, NULL, NULL, 'message', '{ "content": "We should only do this if the options are not already selected because of the URL.\n\nIn the file named `TaskCreate.tsx`, we can use `useEffect` along with `setValue`. Don''t use `getValues`." }'),
  (7, NOW(), 2, NULL, NULL, NULL, 'origin', '{ "orgId": 1 }'),
  (7, NOW(), 2, 1, NULL, NULL, 'repo', '{}'),
  (7, NOW(), 2, NULL, 10, NULL, 'bot', '{}'),
  (7, NOW(), NULL, 1, 10, NULL, 'proposal', '{ "prId": 104, "prNumber": 4, "prTitle": "Auto-select bot & repo if none specified", "prState": "open", "prMerged": false, "prHead": "automa:amp/7", "prBase": "master" }'),
  (8, NOW(), NULL, NULL, NULL, NULL, 'message', '{ "content": "In `AuthLogin` component, when the user clicks on any of the login buttons, we want to send the \"User Login Attempted\" analytic event to track that the user has attempted a login." }'),
  (8, NOW(), NULL, NULL, NULL, NULL, 'origin', '{ "integration": "linear", "organizationId": "aa0479aa-f603-4508-8669-e283bca5a17f", "organizationName": "Automa", "teamId": "7b9f50fa-75b4-43bd-9a0a-0e0994f0ccd9", "teamKey": "DEMO", "teamName": "Demo", "userId": "db18fe9b-d550-44c5-816a-49ac71fccce9", "userName": "Pavan Sunkara", "userEmail": "pavan.sunkara@automa.app", "issueId": "cfb003a0-5c42-48da-b34e-ebbacb9282bb", "issueTitle": "Track \"User Logged In\" event", "issueIdentifier": "DEMO-11", "commentId": "661237eb-3f3d-4bb8-ad22-9245aff0a5d9" }'),
  (8, NOW(), NULL, 1, NULL, NULL, 'repo', '{}'),
  (8, NOW(), NULL, NULL, 8, NULL, 'bot', '{}'),
  (9, NOW(), NULL, NULL, NULL, NULL, 'message', '{ "content": "In `AuthLogout` component, when the user clicks on the logout button, we want to send the \"User Logout Attempted\" analytic event to track that the user has attempted a logout." }'),
  (9, NOW(), NULL, NULL, NULL, NULL, 'origin', '{ "integration": "jira", "organizationId": "373e3aed-a60d-4a6d-9fd1-d136bc545eaa", "organizationUrl": "https://automa-demo.atlassian.net", "organizationName": "automa-demo", "projectId": "10000", "projectKey": "DEMO", "projectName": "Demo", "issuetypeId": "10002", "issuetypeName": "Task", "userId": "712020:3dd57004-4041-4aca-ab80-ced34cc711ab", "userName": "Pavan Sunkara", "userEmail": "pavan.sunkara@automa.app", "issueId": "10000", "issueTitle": "Track \"User Logged Out\" event", "issueKey": "DEMO-1", "commentId": "10040" }'),
  (9, NOW(), NULL, 1, NULL, NULL, 'repo', '{ "integration": "jira", "userId": "712020:3dd57004-4041-4aca-ab80-ced34cc711ab", "userName": "Pavan Sunkara", "userEmail": "pavan.sunkara@automa.app" }'),
  (9, NOW(), NULL, NULL, 8, NULL, 'bot', '{ "integration": "jira", "userId": "712020:3dd57004-4041-4aca-ab80-ced34cc711ab", "userName": "Pavan Sunkara", "userEmail": "pavan.sunkara@automa.app" }'),
  (10, NOW(), 1, NULL, NULL, NULL, 'origin', '{ "orgId": 1 }'),
  (10, NOW(), NULL, NULL, NULL, 5, 'activity', '{}');
